...
class Avtomobile:
    ...
    __repr__() -> str: Возвращает строку, представляющую объект класса Avtomobile.
    """
    ...
    def __repr__(self) -> str:
        """
        Возвращает строку, представляющую объект класса Avtomobile.
        """
        return f"Avto('{self.brand}', '{self.model}', {self.year}, '{self.color}', {self.mileage})"


class Truck(Avtomobile):
    """
    ...
    def __str__(self) -> str:
        """
        Возвращает brand model year, Грузоподъемность: self.carrying_capacity т..
        """
    ...
    def __repr__(self) -> str:
        ...
        :return: brand, model, year, self.carrying_capacity
        """
        return f"Truck('{self.brand}', '{self.model}', {self.year}, {self.carrying_capacity})"


class Minivan(Avtomobile):
    ...
    def carry_passengers(self, num_passengers: int) -> Any:
        ...
        if num_passengers <= self.passengers:
            print(
                f"{num_passengers} пассажиров были успешно перевезены в минивэне, осталось {self.passengers - num_passengers} свободных мест")
            return f'Перевозка состоялась'
        else:
            print(
                f"Минивэн не может перевезти такое количество пассажиров. Сверх допустимого колличества -> {num_passengers - self.passengers} человек")
            return f'Перевозка не состоялась'

    def move(self) -> str:
        """Перемещает мультивэн.
        return: Минивэн brand model перевозит passengers и движется по дороге.
        """
        return f"Минивэн {self.brand} {self.model} перевозит {self.passengers} пассажиров и движется по дороге."


if __name__ == '__main__':
    # Создание объектов класса Avtomobile
    avto1 = Avto('Nissan', 'GT-R', 2018, 'red', 2000)
    avto2 = Avto('Toyota', 'Camry', 2020, 'blue', 4000)

    # Тестирование методов класса Avtomobile
    ...
    print(minivan1.move())  # Движение по дороге с указанным количеством пассажиров.